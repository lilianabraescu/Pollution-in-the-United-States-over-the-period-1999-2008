...
```{r TotalEmissions, echo=TRUE, message=FALSE}
...
```{r BaltimoreEmissions, echo=TRUE, message=FALSE}
...
```{r BaltimoreTypeEmissions, echo=TRUE, message=FALSE}
Baltimore <- NEI[NEI$fips == "24510", ]
...
```{r CoalEmissions, echo=TRUE, message=FALSE}
...
```{r BaltimoreVehicles, echo=TRUE, message=FALSE}
SCC_Vehicles <- SCC %>%
...
### 6. Compare emissions from motor vehicle sources in Baltimore City with emissions from motor vehicle sources in Los Angeles County, California (fips==06037). Which city has seen greater changes over time in motor vehicle emissions?
```{r Comparison, echo=TRUE, message=FALSE}
SCC_Vehicles <- SCC %>%
        filter(grepl('[Vv]ehicle', SCC.Level.Two)) %>%
        select(SCC, SCC.Level.Two)
